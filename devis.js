...
const dateDevisInput = document.getElementById('date-devis');
const clientNomInput = document.getElementById('client-nom');
const clientAdresseInput = document.getElementById('client-adresse');
const numDevisDisplay = document.getElementById('num-devis-display');
...
let currentNumDevis = '...';
function updatenumDevisDisplay() {
    if (numDevisDisplay) numDevisDisplay.textContent = currentNumDevis;
}
updatenumDevisDisplay();

// Fetch current N° Bon from Google Sheets
async function fetchnumDevis() {
    try {
        const response = await fetch(GOOGLE_SHEETS_WEB_APP_URL + "?action=getnumDevis");
        const data = await response.json();
        if (data && data.numDevis) {
            currentNumDevis = parseInt(data.numDevis);
            updatenumDevisDisplay();
            localStorage.setItem('num-devis', currentNumDevis);
        }
    } catch (e) {
        console.error("Erreur lors de la récupération du numéro de bon:", e);
        // Fallback to local storage if network fails
        currentNumDevis = parseInt(localStorage.getItem('num-devis')) || 100230;
        updatenumDevisDisplay();
    }
}
fetchnumDevis();
...
dateDevisInput.value = formatDateFR(new Date());
dateDevisInput.readOnly = true;
dateDevisInput.style.cursor = 'pointer';
...
function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        cell.addEventListener('click', () => {
            ...
            dateDevisInput.value = formatDateFR(selected);
            closeCalendar();
        });
...
function openCalendar() {
    ...
    const parts = dateDevisInput.value.split('/');
    if (parts.length === 3) {
...
dateDevisInput.addEventListener('click', openCalendar);
...
btnPdf.addEventListener('click', async function () {
    ...
    const payload = {
        type: 'devis',
        dateDevis: dateDevisInput.value,
        numDevis: currentNumDevis,
    ...
    try {
        ...
        if (result.status === "Success") {
            ...
            if (result.nextNumDevis) {
                currentNumDevis = parseInt(result.nextNumDevis);
            } else {
                currentNumDevis++;
            }
            localStorage.setItem('num-devis', currentNumDevis);
